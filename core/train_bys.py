...
def load_sql_graphs_pyg(csv_path, dbname, user, password, host, port, start_idx=0, end_idx=8877):
    ...
    return sql_graphs_pyg, df['improvement'].values, attribute_dict
...
def main():
    data_loader_file = '/tmp/job_data_loader.pkl'
    data_loader = None

    # 检查是否已有保存的DataLoader
    if not os.path.exists(data_loader_file):
        # 读取数据并设置数据库
        sql_graphs_pyg, label_list, attribute_dict = load_sql_graphs_pyg(
        ...
        print('len(label_list) 需要是batch_size的倍数:', len(sql_graphs_pyg))
    ...
    print("************",data_loader.dataset[0].x.shape[1],
        data_loader.dataset[0].edge_attr.shape[1],
    ...
    for epoch in range(50):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            model_save_path = f'/home/ubuntu/project/mayang/LOGER/core/infer_model/tpcds10/trained_model_epoch_{epoch + 1}.pth'  # 选择保存的路径和文件名
...
def load_rainbow_model_and_predict(query, query_plan, index_config):
    """
    加载保存的模型并进行预测。
    ...
    """
    database.setup(dbname='imdbload',
            user='postgres',
            password='password',
            host='127.0.0.1', cache=False)
    model_path="/home/ubuntu/project/mayang/LOGER/core/infer_model/tpcds10/trained_model_epoch_10.pth"
    # 读取 pickle 文件
    with open('/home/ubuntu/project/mayang/LOGER/core/infer_model/job/attribute_dict.pkl', 'rb') as f:
        attribute_dict = pickle.load(f)
    # 加载tpcds10模型参数201 15 44
    model = ImprovementPredictionModelGNN(
        201,15,
        graph_embedding_size=32,
        config_vector_size=44
    )
    model.load_state_dict(torch.load(model_path))
    model.eval()

    # 准备输入数据
    g = dgl_node_and_edge_vectorization(query, index_config, query_plan, attribute_dict)  # 假设你已经有了这个函数
    pyg_data = dgl_to_pyg(g, 0)  # 假设你已经有了这个函数，并且可以处理没有label的情况

    # 调用模型进行预测
    with torch.no_grad():
        prediction = model.predict(pyg_data.x, pyg_data.edge_attr, pyg_data.edge_index, pyg_data.configuration_vector, pyg_data.batch).item()

    return prediction

# 假设你已经有了 attribute_dict 和其他必要的数据
# model_path = '/path/to/your/model.pth'
# query = 'your sql query here'
# index_config = 'your index config here'
# prediction = load_model_and_predict(model_path, query, index_config, attribute_dict)
